...
def listar_usuarios() -> List[dict]:
    ...
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute("SELECT id, email, nome, is_admin, papel, ativo, criado_em AS data_criacao, termos_aceitos_em FROM usuarios WHERE ativo = TRUE ORDER BY criado_em DESC")
        rows = cur.fetchall()
        cur.close()
        conn.close()
